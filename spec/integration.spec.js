...
xdescribe('Integration tests:', () => {// eslint-disable-line jasmine/no-disabled-tests
    ...
    describe('Client and Server connectivity:', () => {
        ...
        it('should transfer README.md file to client', () => {
            let basePath = __dirname.split('/');
            basePath.pop();
            basePath = basePath.join('/');
            ...
            const clientPath = path.join(basePath, 'Client');
            const execFile = path.join(clientPath, 'request');
            cp.execSync(`node ${execFile} -t ${ip} -p 3300 -f test/ -ht README.md`);

            const dirPath = path.join(clientPath, 'test');
            expect(utils.isFileExistsInDirectory(dirPath, 'README.md')).toBe(true);
        });
        ...
        it('should transfer index.html file to client', () => {
            ...
            const clientPath = path.join(basePath, 'Client');
            const execFile = path.join(clientPath, 'request');
            cp.execSync(`node ${execFile} -t ${ip} -p 3300 -f test/ -ht index.html`);
    ...
    describe('Client, CDN and Server connectivity:', () => {
        ...
        it('should transfer traffic.png file to client trough CDN', () => {
            let basePath = __dirname.split('/');
            basePath.pop();
            basePath = basePath.join('/');
            ...
            const clientPath = path.join(basePath, 'Client');
            const execFile = path.join(clientPath, 'request');
            cp.execSync(`node ${execFile} -t ${ip} -p 4400 -f test/ -ht traffic.png`);
